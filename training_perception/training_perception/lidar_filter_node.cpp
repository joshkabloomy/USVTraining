...
class LidarFilterNode : public rclcpp::Node {
    ...
    public:
        LidarFilterNode() : Node("perception_lidar_filter") {
            ...
            m_points_sub = this->create_subscription<sensor_msgs::msg::PointCloud2>(
                "input", 10, std::bind(&LidarFilterNode::points_sub_callback, this, std::placeholders::_1)
            );
            m_nonground_pub = this->create_publisher<sensor_msgs::msg::PointCloud2>(
                "/perception/lidar/points_filtered", 10
            );
        }
};
